...
static void gapsize(const Arg *arg); /* gaps */
...
static void movestack(const Arg *arg); /* move stack */
...
manage(Window w, XWindowAttributes *wa)
{
	...
	updatesizehints(c);
	updatewmhints(c);
        c->x = c->mon->mx + (c->mon->mw - WIDTH(c)) / 2; /* centered float */
...
void
spawndwmbar(const Arg *arg)
{
        const char *dwmbarcmd[2] = { "dwm-bar", NULL };
        Arg dwmbararg = {.v = dwmbarcmd };
        spawn(arg);
        spawn(&dwmbararg);
}
